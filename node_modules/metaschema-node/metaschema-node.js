...
a2xml:function(n,a){if(n.indexOf('[]')>-1){n=n.replace('[]','')}var xml='';for(var i=0;i<a.length;i++){if(Array.prototype.isPrototypeOf(a[i])){console.log(n);xml+=this.a2xml(n,a[i]);}else if(String.prototype.isPrototypeOf(a[i])){xml+='<'+n+'>'+a[i]+'</'+n+'>'}else if(typeof(a[i]=='object')){xml+=this._o2xml(n,a[i]);}else{xml+=this.v2xml(n,a[i]);}}return xml;},

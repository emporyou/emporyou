...
MongoClient.connect(Metaschema.config.mongoUrl,function(err,db){															if(err){				 return res.jend(req,res,'error',err);}
	...
	db.collection(cName).drop(function(err,rows){																				if(err){db.close();return res.jend(req,res,'error',err);}res.jsonout.serveroutput[res.jsonout.serveroutput.length]=rows;
...
{_id:idapp,_p:idroot,_k:idroot,name:'app',rel:[],meta:[],url:false,desc:'default apps node',system:true,_owned_by:idadmin,created:new Date(),_created_by:idadmin,modified:new Date(),_modified_by:idadmin,users:[]},				
					],function(err,rows){																								db.close();if(err){return res.jend(req,res,'error',err);}res.jsonout.serveroutput[res.jsonout.serveroutput.length]=rows;			
							/*todo:create indexes*/

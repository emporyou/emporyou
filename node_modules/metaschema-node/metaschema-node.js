...
Metaschema={CONFIG:{port:8080,ip:'0.0.0.0',dirname:'./static',defaultfile:'index.html',
	mongoUrl:'mongodb://localhost:27017/metaschema',collectionName:'dat',
	findParameter:'jfind',dataParameter:'jdata',viewParameter:'jview',
	pageParameter:'page',replaceToken:'replace_jdata',separatorParameter:',',urltorecord:{redirect:'/?',parameter:'_id'},
	rootID:ObjectID('000000000000000000000000'),
	userID:ObjectID('000000000000000000000001'),
	 tagID:ObjectID('000000000000000000000002'),
	 binID:ObjectID('000000000000000000000003'),
    docID:ObjectID('000000000000000000000004'),
  adminID:ObjectID('000000000000000000000005'),
defaultDB:[],
   init:function(){this.defaultDB=[];var x=this.rootID;
		this.defaultDB[0]=new Metaschema.Doc(x,x,x,x,'root','root node',false,true);
		this.defaultDB[1]=new Metaschema.Doc(this.userID,x,x,x,'user','users node',false,true);
		this.defaultDB[2]=new Metaschema.Doc(this.tagID,x,x,x,'tag','tags node',false,true);
		this.defaultDB[3]=new Metaschema.Doc(this.binID,x,x,x,'bin','bins node',false,true);
		this.defaultDB[4]=new Metaschema.Doc(this.docID,x,x,x,'doc','docs node',false,true);
		this.defaultDB[5]=new Metaschema.Doc(this.adminID,this.userID,this.userID,x,'admin','default admin',false,true);
}},aabb:function(a,b){if(a){if(b){for(k in b){a[k]=b[k]}}return a}else{return b}},
apply:function(oo){this.CONFIG.init();if(oo){this.CONFIG=this.aabb(this.CONFIG,oo);}},
...
_o2xml:function(n,o){if(n.indexOf('[]')>-1){n=n.replace('[]','')}if(n.indexOf('_')==0){n=n.substr(1);return '<'+n+'>'+o+'</'+n+'>'}var xml='<'+n+'>';var pr;
...
a2xml:function(n,a){if(n.indexOf('[]')>-1){n=n.replace('[]','')}var xml='';for(var i=0;i<a.length;i++){if(Array.prototype.isPrototypeOf(a[i])){xml+=this.a2xml(n,a[i]);}else if(typeof(a[i]=='object')){xml+=this._o2xml(n,a[i]);}else{xml+=this.v2xml(n,a[i]);}}return xml;},
v2xml:function(n,v){if(n.indexOf('[]')>-1){n=n.replace('[]','')}if(typeof(v)=='function'){return ''}var cd=false;if(typeof(v)=='string'){cd=true;}if(cd){return '<'+n+'><![CDATA['+v+']]></'+n+'>';}else{return '<'+n+'>'+v+'</'+n+'>';}},
/*-------------------------------------------------------------------------------- context adaptation */
...
findQ:function(req,res){var Q={};
	var q=req.item(Metaschema.CONFIG.findParameter);
	if(q){try{Q=JSON.parse(q);}catch(ex){Q={};return res.jend(req,res,'error','could not parse json');}}
   Q=Metaschema.findid(req,Q,'id');Q=Metaschema.findid(req,Q,'p');Q=Metaschema.findid(req,Q,'k');
	q=req.item('rel');if(q){Q.rel=q}q=req.item('role');if(q){Q.role=q}var qq;var qi=0;
	if(Q.rel){if(!Array.prototype.isPrototypeOf(Q.rel)){var rr=[];qq=Q.rel.split(Metaschema.CONFIG.separatorParameter);for(qi=0;qi<qq.length;qi++){if(qq[qi]!=''){rr[rr.length]=ObjectID(Metaschema.zeropad(qq[qi]))}}Q.rel={"$elemMatch":{"key":{"$in":rr}}}}}
...
findid:function(req,Q,prop){var _prop='_'+prop;var q=req.item(prop);if(!q){return Q}
	if(q.indexOf(Metaschema.CONFIG.separatorParameter)<0){Q[_prop]=ObjectID(Metaschema.zeropad(q));return Q}
	else{var qq=q.split(Metaschema.CONFIG.separatorParameter);var rr=[];
		for(var r=0;r<qq.length;r++){if(qq[r]){if(qq[r]!=''){rr[rr.length]=ObjectID(Metaschema.zeropad(qq[r]))}}}
		Q[_prop]={$in:rr};return Q;}},
zeropad:function(s){var n=24-s.length+1;if(n<1){return s}return Array(+(n > 0 && n)).join("0")+s;},
/*------------------------------------------------------------------------------- set query generator */
...
get:function(req,res,next){
   ...
   Q=Metaschema.findid(req,Q,'id');Q=Metaschema.findid(req,Q,'p');Q=Metaschema.findid(req,Q,'k');
	q=req.item('rel');if(q){Q.rel=q}q=req.item('role');if(q){Q.role=q}var qq;var qi=0;
	if(Q.rel){if(Q.rel.split){var rr=[];qq=Q.rel.split(Metaschema.CONFIG.separatorParameter);for(qi=0;qi<qq.length;qi++){if(qq[qi]!=''){rr[rr.length]=ObjectID(Metaschema.zeropad(qq[qi]))}}Q.rel={"$elemMatch":{"key":{"$in":rr}}}}}
	if(Q.role){var rgx=new RegExp('.*'+Q.role.replace(/ /g,'.*')+'.*');if(!Q.rel){Q.rel={$elemMatch:{r:rgx}}}else{Q.rel.$elemMatch.r=rgx}delete Q.role;}
	var pQ=req.item(Metaschema.CONFIG.viewParameter);
	if(!pQ){pQ={}}else if(pQ=='autocomplete'){pQ={_id:1,name:1}}
	else{try{pQ=JSON.parse(pQ)}catch(ex){pQ={};return res.jend(req,res,'error',' error parsing JSON - view parameter must be \'autocomplete\' or some JSON string');}}
	//----------------------------------------------------------------------------------- query execution
...
set:function(req,res){		
	...
	/*TODO*/
	var fQ=Metaschema.findQ(req,res);
...
//------------------------------------------------------------------------------------------ ADD SERVICE
//------------------------------------------------------------------------------------------------------
add:function(req,res){var Q={};
	var q=req.item(Metaschema.CONFIG.dataParameter);
	if(q){try{Q=JSON.parse(q);}catch(ex){Q={};return res.jend(req,res,'error','could not parse json');}}
	//TODO:Ensure fields
	if(Q._id){return res.jend(req,res,'error','could not add document with id already set');}
	if(Q.id){return res.jend(req,res,'error','could not add document with id already set');}
	q=req.item('k');if(q){Q.k=q}
	q=req.item('p');if(q){Q.p=q}
	if(Q.k){Q._k=ObjectID(Metaschema.zeropad(Q.k));}else{Q._k=ObjectID('000000000000000000000010');}delete Q.k
	if(Q.p){Q._p=ObjectID(Metaschema.zeropad(Q.p));}else{Q._p=ObjectID('000000000000000000000010');}delete Q.p
	Q.created=new Date();
	Q.modified=new Date();
	//TODO:Ensure sec
	//----------------------------------------------------------------------------------- query execution
MongoClient.connect(Metaschema.CONFIG.mongoUrl,function(err,db){															if(err){				 return res.jend(req,res,'error',err);}
	db.collection(Metaschema.CONFIG.collectionName).insert(Q,function(err,rows){							db.close();if(err){return res.jend(req,res,'error',err);}res.jsonout.serveroutput[res.jsonout.serveroutput.length]=rows;
		res.jend(req,res,'confirm','command was executed');
});});},
//------------------------------------------------------------------------------------------------------
//------------------------------------------------------------------------------------------ DEL SERVICE
//------------------------------------------------------------------------------------------------------
del:function(req,res){Q=Metaschema.findQ(req,res);res.jsonout.delquery=Q;
	//----------------------------------------------------------------------------------- query execution
MongoClient.connect(Metaschema.CONFIG.mongoUrl,function(err,db){															if(err){				 return res.jend(req,res,'error',err);}
	db.collection(Metaschema.CONFIG.collectionName).remove(Q,function(err,rows){										db.close();if(err){return res.jend(req,res,'error',err);}res.jsonout.serveroutput[res.jsonout.serveroutput.length]=rows;
...
MongoClient.connect(Metaschema.CONFIG.mongoUrl,function(err,db){															if(err){				 return res.jend(req,res,'error',err.message);}
//	db.collection('user').drop(function(err,rows){																				if(err){db.close();return res.jend(req,res,'error',err);}res.jsonout.serveroutput[res.jsonout.serveroutput.length]=rows;
	db.collection(cName).drop(function(err,rows){																				
		db.createCollection(cName,function(err,rows){																	if(err){db.close();return res.jend(req,res,'error',err.message);}
			db.collection(cName).insert(Metaschema.CONFIG.defaultDB,function(err,rows){						db.close();if(err){return res.jend(req,res,'error',err.message);}
							/*todo:create indexes*/
...
metaframe:function(req,res){
   var fn=req.item(Metaschema.CONFIG.pageParameter);
	  ...
	  var json=req.item(Metaschema.CONFIG.dataParameter);
	  fs.createReadStream(fn).pipe(replaceStream(Metaschema.CONFIG.replaceToken,json)).pipe(res);
...
Metaschema._recursefindandserve(req,res,next,n)}}})}})})},
...
//------------------------------------------------------------------------------------------------------
Metaschema.Doc=function(id,p,k,o,n,d,u,s){this._id=ObjectID(id);this._p=ObjectID(p);this._k=ObjectID(k);this.name=n;this.desc=d;this.url=u;this.system=s;
this.created=new Date();this.modified=new Date();this._created_by=o;this._owned_by=o;this._modified_by=o;};

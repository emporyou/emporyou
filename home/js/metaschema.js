...
/*-----------------------------------------------------------------------------------------------------  XML RENDER TOOL */
 ...
 $._dorendercontrol3=function(req,oo){var ct=req.getResponseHeader('content-type');if(!ct){if((req.responseText.indexOf('<?xml version="1')>0)&&(req.responseText.indexOf('<?xml version="1')<20)){ct='xml'}}if(!ct){ct='json'}if(ct.indexOf('xml')<0){oo.dataXML=ooo.JSON2xmldoc(JSON.parse(req.responseText));if(ooo.ixml(oo.dataXML).indexOf("Below")>-1){throw new Error(oo.dataXML.innerHTML)}

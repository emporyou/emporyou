...
/*-----------------------------------------------------------------------------------------------------  XML RENDER TOOL */
 ...
 $._dorendercontrol3=function(req,oo){var ct=req.getResponseHeader('content-type');if(ct.indexOf('xml')<0){oo.dataXML=ooo.JSON2xmldoc(JSON.parse(req.responseText))}else{oo.dataXML=ooo.parsexml(req.responseText);}var s=$.syncrender(oo.target,oo.templateXML.documentElement,oo.dataXML.documentElement,oo.mode||'normal');if(oo.onfinish){oo.onfinish(s,oo)}};

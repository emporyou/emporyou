...
/*-----------------------------------------------------------------------------------------------------  XML RENDER TOOL */
 ...
 $._dorendercontrol1=function(req,oo){var ct=req.getResponseHeader('content-type');if(ct.indexOf('xml')<0){oo.templateXML=ooo.JSON2xmldoc(JSON.parse(req.responseText))}else{oo.templateXML=ooo.parsexml(req.responseText);}$._dorendercontrol2(false,oo);};
 ...
 $._dorendercontrol3=function(req,oo){console.log(req.getResponseHeader('content-type')+' & '+req.getResponseHeader('Content-Type'))var ct=req.getResponseHeader('content-type');if(ct.indexOf('xml')<0){oo.dataXML=ooo.JSON2xmldoc(JSON.parse(req.responseText))}else{oo.dataXML=ooo.parsexml(req.responseText);}window.lastJob=oo;var s=$.syncrender(oo.target,oo.templateXML.documentElement,oo.dataXML.documentElement,oo.mode||'normal');if(oo.onfinish){oo.onfinish(s,oo)}};
